...
import { createTimetableClass, getTimetableAdminData, updateTimetableClass } from "@/lib/data";
...
export async function PATCH(request: Request) {
  ...
  if (!session.access.isFullAccess) {
    return NextResponse.json({ error: "Only admins can edit timetable classes." }, { status: 403 });
  }
  ...
  try {
    ...
    const updated = await updateTimetableClass({
      originalClassCode: String(body.originalClassCode ?? ""),
      originalClassName: String(body.originalClassName ?? ""),
      classCode: String(body.classCode ?? ""),
      className: String(body.className ?? ""),
      school: "Primary",
      designation: streamType,
    ...
    return NextResponse.json({ classRecord: updated });
  } catch (error) {
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Unable to update timetable class." },
      { status: 400 }
    );
  }
}
